...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp_cam):
      ...
      if self.enable_mads:
        ...
        if self.cruise_buttons[-1] == 3:
          ...
          if self.mads_hold_counter > 50:
            self.mads_hold_counter = 0
            if not self.mads_hold_once:
              self.madsEnabled = not self.madsEnabled
            self.mads_hold_once = True
        else:
          self.mads_hold_counter = 0
          self.mads_hold_once = False
    ...
    if not self.CP.pcmCruise or not self.enable_mads:
    ...
    if self.madsEnabled and self.CP.latActive:
