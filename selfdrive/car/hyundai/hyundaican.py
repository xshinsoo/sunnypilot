...
def create_lkas11(packer, frame, car_fingerprint, apply_steer, steer_req,
                  ...
                  disengage_from_brakes, below_lane_change_speed, disengage_blink):
  values = lkas11
  ...
  if car_fingerprint in (CAR.SONATA, CAR.PALISADE, CAR.KIA_NIRO_EV, CAR.KIA_NIRO_HEV_2021, CAR.SANTA_FE,
    ...
    values["CF_Lkas_FcwOpt_USM"] = 2 if lat_active else 2 if disengage_blink else 1 if\
...
def create_lfahda_mfc(packer, lat_active, disengage_from_brakes, below_lane_change_speed, disengage_blink, hda_set_speed=0):
  values = {
    "LFA_Icon_State": 2 if lat_active else 3 if disengage_blink else 1 if (disengage_from_brakes or below_lane_change_speed) else 0,
...
def create_acc_commands(packer, enabled, accel, jerk_upper, jerk_lower, idx, lead_visible, set_speed, stopping, gas_pressed, main_enabled, escc):
  commands = []
